...
export const MyBookings = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setIsLoading(true);
                const response = await axios.post("http://localhost:3000/api/bookings/getBookingsByUserId", { userId: user._id });
    ...
    async function cancelBooking(bookingId, courtId) {
        try {
            setIsLoading(true);
            const response = await axios.post("http://localhost:3000/api/bookings/cancelBooking/", { bookingId, courtId });

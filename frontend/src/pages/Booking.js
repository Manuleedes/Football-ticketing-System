...
const Booking = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            try {
                setIsLoading(true);
                const response = await axios.post("http://localhost:3000/api/courts/getCourtById", { courtId });
    ...
    async function payNow(token) {
        ...
        try {
            setIsLoading(true);
            const response = await axios.post("http://localhost:3000/api/bookings/bookingCourt", bookingDetails);
